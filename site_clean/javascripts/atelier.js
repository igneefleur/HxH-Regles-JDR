...
(function () {
  ...
  function catColor(cat) { return "var(" + (CAT_VARS[cat] || "--fg-cablage") + ")"; }
  function catName(cat) { return cat ? (CAT_LABEL[cat] || cat) : "Câblage"; }
  ...
  function devBadges(dxByCat) {
    return Object.keys(dxByCat).filter(function (cat) { return dxByCat[cat]; }).map(function (cat) {
      var isc = cat === "câblage";
      return h("span", { class: "nf-dev-b", style: "background:" + (isc ? "var(--fg-cablage)" : catColor(cat)) }, [dxByCat[cat] + " " + (isc ? "DI" : devLabel(cat))]);
    });
  }
  ...
  // Cible par défaut selon le socle : une créature/un objet DÉTACHÉ (conjuration/émission de
  // créature, conjuration d'objet) porte sur l'entité elle-même, cible extérieure qui exige un
  // relais de maintien ; transmutation d'arme (l'objet fait corps) et manipulation (l'emprise
  // prolonge le porteur) restent « Soi-même », donc maintien direct.
  function defaultCible(ctype) {
    if (ctype === "bête" || ctype === "bête-e") return "La créature";
    if (ctype === "objet") return "L'objet";
    return "Soi-même";
  }
  // Cible EFFECTIVE d'une capacité : le défaut du socle ; un déclencheur qui la vise et retient
  // « Défenseur » la fait porter sur le défenseur de l'attaque. Une capacité cataloguée garde
  // sa cible propre (par défaut « Soi-même », celui qui la conjure).
  function cibleOf(state, start) {
    var over = null;
    forEachModule(state, function (node) {
      if (isDeclencheur(moduleDef(node.defId)) && refTargetOf(state, node.id) === start.id) {
        var c = declencheurCible(node); if (c) over = c;
      }
    });
    if (over) return /défenseur/i.test(over) ? "Défenseur de l'attaque" : "Soi-même (attaquant)";
  ...
  function chainReport(state, start) {
    ...
    if (start.defId) {   // socle piloté par une fiche (bête) : son coût propre entre dans la capacité
      var sc = moduleCost(start), sk = moduleDef(start.defId).category || "câblage";
    ...
    ch.modules.forEach(function (node) {
      ...
      var kk = def.category || "câblage"; dxByCat[kk] = (dxByCat[kk] || 0) + c.di;
  ...
  function powerReport(state) {
    ...
    for (var cat in dxByCat) {
      if (cat === "câblage") { diByCat[cat] = dxByCat[cat]; diTotal += dxByCat[cat]; }
      else {
  ...
  function mount(root) {
    ...
    function openSearch(opts) {
      ...
      function renderList() {
        ...
        shown.forEach(function (r, i) {
          var it = r.it, color = it.ty ? tyColor(it.ty) : (it.cat ? catColor(it.cat) : "var(--fg-cablage)");
    ...
    // case AE d'un bandeau de coûts : AEG (socle/câblage) ou AEL (module) selon la portée du module.
    ...
    // Une seule colonne d'affinité : AEG pour les socles/câblage (décalage global),
    ...
    function tblGrid(title, valueEl, st, devLbl, caracLbl, scope) {
      return h("div", { class: "nf-tgrid" }, [
        h("span", { class: "h name" }, [title]),
        h("span", { class: "h sep" }, [devLbl]), h("span", { class: "h" }, [caracLbl]), h("span", { class: "h sep" }, ["UA"]), h("span", { class: "h" }, ["MA"]), h("span", { class: "h sep" }, [aeLabel(scope)]),
    ...
    function groupSection(n, def, grid, grp, key, devLbl, soc) {
      ...
      if (grp.cumulable) {
        ...
        var cells = [
          ...
          h("span", { class: "h sep", style: "grid-area:1/4" }, ["UA"]),
        ...
        sec.appendChild(tblGrid(title, selEl, selRow, devLbl, caracLbl, scope));
    ...
    function buildStartNode(n) {
      ...
      body.appendChild(field("Cible", h("div", { class: "nf-cible" }, [cibleOf(state, n)])));
      ...
      if (sd) {
        ...
        var sc = moduleCost(n);
        body.appendChild(costStrip([[devLabel(sd.category), sc.di], ["UA", sc.ua], ["MA", sc.ma], aeStripCell(sd, sc, n)]));
      }
    ...
    function buildArmeNode(n) {
      ...
      [["Dégâts", f.degats], ["Type", f.type], ["Portée", f.portee], ["Mains", f.mains], ["Mod.", f.mod], ["AM", f.am], ["Illégalité", f.illeg], ["Propriétés", f.props]].forEach(function (r) {
    ...
    function buildModNode(n) {
      ...
      var c = moduleCost(n);
      body.appendChild(costStrip([[devLabel(def.category), c.di], ["UA", c.ua], ["MA", c.ma], aeStripCell(def, c, n)]));
    ...
    function buildEndNode(n) {
      ...
      if (!r) body.appendChild(h("div", { class: "nf-empty-hint" }, ["Reliez une capacité à l'entrée pour voir sa fiche."]));
        ...
        body.appendChild(costStrip([["UA", r.ua], [maLbl, r.instant ? "—" : r.maShown]].concat(caracBoxes(r.carByCat))));
  ...
  function drawPalette(filter) {
    ...
    var byCat = {};
    DATA.modules.forEach(function (m) { if (isSocleDefModule(m)) return; var k = m.category || "câblage"; (byCat[k] = byCat[k] || []).push(m); });
    ["renforcement", "émission", "transmutation", "manipulation", "conjuration", "spécialisation", "câblage"].forEach(function (cat) {
      var list = (byCat[cat] || []).filter(function (m) { return !f || m.name.toLowerCase().indexOf(f) !== -1 || (m.description || "").toLowerCase().indexOf(f) !== -1; });
      if (!list.length) return;
      var color = cat === "câblage" ? "var(--fg-cablage)" : catColor(cat);
      ...
      container.appendChild(catBlock(cat, catName(cat === "câblage" ? null : cat), color, items, list.length));
    });
  }
  ...
  function renderInspector(container, state) {
    ...
    pr.reports.forEach(function (r) {
      ...
      card.appendChild(costStrip([["UA", r.ua], [maLbl, r.instant ? "—" : r.maShown]].concat(caracBoxes(r.carByCat))));
    ...
    Object.keys(pr.dxByCat).filter(function (cat) { return pr.dxByCat[cat]; }).forEach(function (cat) {
      var isc = cat === "câblage", dx = pr.dxByCat[cat], di = pr.diByCat[cat], aa = (!isc && arch) ? arch.affinities[cat] : null;
      brk.appendChild(h("div", { class: "nf-devrow" }, [
        h("span", { class: "dot", style: "color:" + (isc ? "var(--fg-cablage)" : catColor(cat)) }, ["●"]),
