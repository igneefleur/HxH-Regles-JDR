...
(function () {
  ...
  function su(id) { return find(DATA.supplementaires, id); }
  ...
  function cost(n) { return n === 0 ? "0" : (n > 0 ? "+" + n : String(n)); }
  ...
  function weaponFiche() {
    return {
      am: state.am, portee: fichePortee(), mains: state.mains, degats: state.degats,
  ...
  function porteeNote(label) { var v = ob("portee"); return noteOf(v, label, 0) || noteOf(v, label, 1); }
  ...
  function lines() {
    ...
    if (state.cone) add("Cône " + state.cone.len + ", " + state.cone.ang,
      (map(ob("portee"), 2)[state.cone.len] || 0) + (map(ob("portee"), 3)[state.cone.ang] || 0));
  ...
  function refresh() {
    ...
    [["Compat. AM", state.am], ["Portée", fichePortee()], ["Mains", state.mains],
  ...
  function chips(parent, rows, set, fmt) {
    ...
    rows.forEach(function (r) {
      ...
      c.addEventListener("click", function () {
        ...
        if (i >= 0) { set.splice(i, 1); c.classList.remove("on"); } else { set.push(r.label); c.classList.add("on"); }
        refresh();
      });
      box.appendChild(c);
    });
  ...
  function buildCone(parent, box) {
    ...
    ob("portee").tables[2].rows.forEach(function (r) { var o = el("option"); o.value = r.label; o.textContent = r.label + " (" + cost(r.cost) + ")"; lenS.appendChild(o); });
    ob("portee").tables[3].rows.forEach(function (r) { var o = el("option"); o.value = r.label; o.textContent = r.label + " (" + cost(r.cost) + ")"; angS.appendChild(o); });
  ...
  function buildForm(grid) {
    ...
    var boxP = chips(cP, ob("portee").tables[0].rows.concat(ob("portee").tables[1].rows), state.portees, function (r) { return r.label + sub + (r.note || "") + "</small>"; });
    buildCone(cP, boxP);
